...
class VideoBERTTransformerModel(nn.Module):
    def __init__(self, input_dim=1280, embed_dim=256, num_heads=8, num_layers=2):
        super().__init__()
        self.projection = nn.Linear(input_dim, embed_dim)
        
        # BERT parameter hooks
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        nn.init.normal_(self.cls_token, std=0.02)
        self.position_embeddings = nn.Embedding(11, embed_dim) # Size 11 = 10 frames + 1 [CLS]
    ...
    def forward(self, x, head='binary'):
        ...
        # Prepend [CLS] token context vector
        cls_tokens = self.cls_token.expand(batch_size, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        
        pos_ids = torch.arange(0, 11, dtype=torch.long, device=x.device).unsqueeze(0)
        x = x + self.position_embeddings(pos_ids)
        
        bert_out = self.transformer_bert(x)
        cls_vector = bert_out[:, 0, :] # Extract index 0 representing the updated CLS token
        
        shared = self.gelu(self.dense_shared(cls_vector))
        if head == 'binary':
            return torch.sigmoid(self.binary_head(shared)).squeeze(-1)
        return self.multiclass_head(shared)

# --- RUN LOOP ---
model = VideoBERTTransformerModel().to(device)
...
print("\nLaunching Shuffled 10-Fold Video-BERT CV Engine...")
...
pd.DataFrame(rep_v_b).transpose().to_csv(output_dir / 'bert_val_metrics_binary.csv')
pd.DataFrame(rep_v_m).transpose().to_csv(output_dir / 'bert_val_metrics_multiclass.csv')
pd.DataFrame(rep_t_b).transpose().to_csv(output_dir / 'bert_final_metrics_binary.csv')
pd.DataFrame(rep_t_m).transpose().to_csv(output_dir / 'bert_final_metrics_multiclass.csv')

save_report_as_png(rep_v_b, "Validation Fold - Video-BERT Binary Metrics", "bert_val_report_binary.png")
save_report_as_png(rep_v_m, "Validation Fold - Video-BERT Multiclass Metrics", "bert_val_report_multiclass.png")
save_report_as_png(rep_t_b, "Test Fold - Video-BERT Binary Metrics", "bert_classification_report_binary.png")
save_report_as_png(rep_t_m, "Test Fold - Video-BERT Multiclass Metrics", "bert_classification_report_multiclass.png")

save_confusion_matrix_png(t_trues_b, t_preds_b, ['Clean', 'Noise'], "Video-BERT Test - Binary Matrix", "bert_confusion_matrix_binary.png", "YlGnBu")
save_confusion_matrix_png(t_trues_m, t_preds_m, all_names, "Video-BERT Test - Multiclass Matrix", "bert_confusion_matrix_multiclass.png", "Purples")
print("Video-BERT processing complete. All assets compiled successfully inside /working directory.")